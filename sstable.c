...
struct SSTable *SSTable_new(char *path)
{
    ...
    while (1)
    {
        ...
        curr_offset += sizeof(uint64_t) + next;
    }
...
struct SSTable *SSTable_new_from_memtable(char *path, struct MemTable *memtable)
{
    ...
    struct RBTNode *records = (struct RBTNode *)malloc(sizeof(struct RBTNode) * memtable->size);
    records = MemTable_to_list(memtable);

    for (int i = 0; i < memtable->size; i++)
    {

        uint64_t key_len = records[i].key_len;
        int64_t value_loc = records[i].value_loc;
        ...
        res = fwrite(records[i].key, sizeof(char), key_len, file);
        if (res != key_len)
        {
            perror("fwrite");
            return NULL;
...
int SSTable_key_cmp(struct SSTableRecord *r, const char *key, size_t key_len)
{
    ...
    // if they are not equal then return comparison state(-1 or 1)
...
int64_t SSTable_get_value_loc(struct SSTable *table, char *key, size_t key_len)
{
    ...
    // In a situation where size is 1, we just run this and not the loop.
    // Also, in a situation where a = m+1 and m+1 is b then exit the loop and run
    // this
    struct SSTableRecord record;
...
//         MemTable_set(memtable, (char *) &bytes, 4, i * 128);
//     }
